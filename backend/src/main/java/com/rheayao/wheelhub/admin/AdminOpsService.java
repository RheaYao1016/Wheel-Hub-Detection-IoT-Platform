...
public class AdminOpsService {
    ...
    private final CopyOnWriteArrayList<AlertRecord> alerts = new CopyOnWriteArrayList<>(seedAlerts());
    private final CopyOnWriteArrayList<ImportBatch> importBatches = new CopyOnWriteArrayList<>(seedImports());
    ...
    public boolean deleteImport(String id) {
        return importBatches.removeIf(item -> item.id().equals(id));
    }
    ...
    private static List<AlertRecord> seedAlerts() {
        return List.of(
            new AlertRecord("AL-2025-0311-01", "2025-03-11 08:42:11", "ST-01", "高", "圆跳动超限 0.32mm，超出 0.25mm 阈值", "待处理"),
    ...
    private static List<ImportBatch> seedImports() {
        return List.of(
            new ImportBatch("IMP-20250312-001", "2025-03-12-shiftA.csv", 2411725L, 5200, 8800L, "成功", "2025-03-12 09:20:00", "李雷", "夜班导入完毕", null,
                "[2025-03-12 09:20:00] 李雷 提交导入任务：2025-03-12-shiftA.csv\n[2025-03-12 09:20:00] 系统校验字段完整性\n[2025-03-12 09:20:00] 状态：成功"),
            new ImportBatch("IMP-20250312-002", "2025-03-12-recheck.csv", 1153433L, 1900, 5100L, "部分成功", "2025-03-12 11:10:00", "韩梅梅", "存在 8 条待复核", "校验失败行号：133、155、232",
            ...
            new ImportBatch("IMP-20250310-003", "2025-03-10-lab.csv", 943718L, 1200, 4200L, "失败", "2025-03-10 14:30:00", "张伟", "CSV 列缺失", "缺少 diameter、center 列，已终止导入",
