...
const SIZE_BANDS = [
  { label: "15寸", min: 0, max: 560 },
  { label: "16寸", min: 560, max: 590 },
  { label: "17寸", min: 590, max: 620 },
  { label: "18寸", min: 620, max: 660 },
  { label: "19寸", min: 660, max: Number.POSITIVE_INFINITY },
] as const;
...
function buildMetrics(records: WheelRecord[]): MetricCard[] {
  ...
  return [
    ...
    { label: "平均节拍", value: "53.8s", note: "相较昨日更平稳", delta: "-4.6%", trend: "down" },
...
function buildSensors(records: WheelRecord[]): SensorSnapshot[] {
  const diameterAverage =
    records.reduce((sum, record) => sum + record.diameter, 0) / Math.max(records.length, 1);
...
export async function getCommandCenterSnapshot(): Promise<CommandCenterSnapshot> {
  ...
  return {
    headline: {
      ...
      description: "面向检测、数字孪生、仓储联动的一体化看板。当前版本支持本地数据文件演示模式与 Prisma 数据库模式双栈运行。",
    },
...
export async function getDigitalTwinSnapshot(): Promise<DigitalTwinSnapshot> {
  ...
  return {
    summary: {
      title: "数字孪生作业单元",
      description: "围绕 3D 模型、工艺流程、传感器与告警闭环展示检测装备的实时画像，方便工艺、设备与算法团队协同排障。",
...
export async function getMonitorSnapshot(): Promise<MonitorSnapshot> {
  ...
  return {
    headline: {
      title: "生产监控与异常追踪",
      description: "联动四路视频窗口、设备健康、趋势曲线与预警事件，用于班组长和运维侧的日常巡检。",
    },
...
export async function getAdminSnapshot(): Promise<AdminSnapshot> {
  ...
  return {
    overview: {
      title: "管理员运营后台",
      description: "提供导入、告警、产线质量与设备利用率的统一入口，适合作为交付版项目的管理驾驶舱。",
    },
    metrics: buildMetrics(records),
